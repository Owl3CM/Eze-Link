    ...
    function createMethod(method: string) {
      return ({ url, body, headers: _headers }: IMethod<Headers>) => {
        ...
        Functions[abortId] = {
          retry: () => {
            setTimeout(() => {
              Functions[abortId].abort();
              (_apiService as any)[method]({ url, body, headers: _headers });
            }, 1);
          },
          abort: () => {
            setTimeout(() => {
        ...
        return new Promise<any>(async (resolve, reject) => {
          try {
            await onRequest?.({ props, Functions } as any);
            const res = await fetch(url, props as RequestInit);
            delete Aborts[abortId];
            if (res.ok) {
              try {
...
type OnRequest<T> = ({ props, Functions }: { props: OnRequestProps<T>; Functions: any[] }) => void;
